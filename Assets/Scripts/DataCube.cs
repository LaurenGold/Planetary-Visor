...
public class DataCube : MonoBehaviour {
    ...
    //Future implementation: dynamically get these from file
    ...
    public static void LoadData() {
        if (dataCube!= null) {
            Debug.Log("Data cube is null");
            return;
        }
        TextAsset dataAsset = Resources.Load("ssa", typeof(TextAsset)) as TextAsset;
        Debug.Log("Data asset loaded:\t" + dataAsset.bytes.Length);
        BinaryReader br = new BinaryReader(new MemoryStream(dataAsset.bytes));
        Debug.Log("Binary Reader Stream loaded:\t" + (br == null));
        ...
        for (int band = spectralBands - 1; band >= 0; band--) {
            for (int row = 0; row < dataHeight; row++) {
                for (int col = 0; col < dataWidth; col++) {
                    //columns are stored in reverse order.
        ...
        // ReadSingle() calls.
        TextAsset dataAsset2 = Resources.Load("ddr", typeof(TextAsset)) as TextAsset;
    ...
    /// <summary>
    /// Loads given Unity TextAsset as bytes file representative of DDR.
    /// This function call allows for v1 PVisor build to retain reference to ddr file.
    /// When building with Resources.Load, the directories are varied across platforms.
    /// TODO: Determine the correct way to load files in Quest builds without having reference before runtime.
    /// </summary>
    public static void LoadData(TextAsset dataAsset, TextAsset dataAsset2) {
        if (dataCube != null) {
            Debug.Log("Data cube is null");
            return;
        }
    ...
    public static Vector2 getColRowFromUnityXZ(float x, float z) {
